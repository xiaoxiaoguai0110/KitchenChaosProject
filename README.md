...
- 优先处理较早订单，并优先安排需要煎制、切制等耗时加工的食材
- 候选柜台按照交付、加工、取餐盘、取原料等行为价值评分，同类目标再结合距离选择
- 实时扫描场景柜台与两名角色手中的食材，并将生食材可加工出的成品计入当前进度
- 预约当前订单与目标食材；真人抢先接手同一食材时，AI 会取消预约并重新规划，避免重复制作
...
| `AIPlayerTargetSelector` | 维护任务预约，并按行为价值和距离选择目标柜台 |
| `AIOrderPlanner` | 计算订单缺口、识别加工进度、确定食材优先级并反查原料 |
...
### v1.16 - AI 任务预约与目标优先级
- AI 目标选择由随机候选升级为“行为价值 + 距离”评分，优先交付、加工和继续半成品
- 优先处理较早订单，并优先制作煎制、切制等加工耗时更长的缺失食材
- 新增订单与食材任务预约，使 AI 在领取、加工和装盘阶段保持一致目标
- 场景柜台以及真人、AI 手中的原料和潜在加工产物统一计入制作进度
- 真人抢先领取或加工预约食材时，AI 会主动放弃原任务并重新规划，减少重复制作
- Windows x64 正式构建通过，成品可正常启动且启动日志无异常
